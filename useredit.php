...
echo "&nbsp;";
$idc = $_GET["id"];
$sorgu = $baglanti->prepare("SELECT COUNT(*) as sayi FROM users WHERE id ='$idc'");
$sorgu->execute();
...
<head>
    <meta charset="utf-8" />
    <title>Kullanıcı Düzenle | Fayu Yönetim Paneli</title>
                            ...
                            <div class="card-body">
                                <div class="px-2 py-3">
                                    ...
                                    <div class="text-center">
                                        ...
                                        <h5 class="text-primary mb-2 mt-4">Kullanıcı Düzenle</h5>
                                        <p class="text-muted"><?= $sonuc["Isim"] ?> kullanıcısını düzenle.</p>
                                    </div>
                                    <?php
                                    if ($_POST) {
                                        if ($_POST["txtPass"] == $_POST["txtPass2"]){
                                            ...
                                            $guncelleSorgu = $baglanti->prepare("UPDATE users set Isim=:Isim, Pass=:Pass where id=:id");
                                            ...
                                            echo "<script> Swal.fire({title:'HATA!', text:'HATA OLUŞTU', icon:'error', confirmButtonText:'Kapat'}).then((value) => {if (value.isConfirmed){window.location.href='useredit.php?id=".$_SESSION["id"]."'}})</script>";
                                        }
                                            if ($guncelle) {
                                                echo '<script type="text/javascript" src="assets/js/sweetalert2.all.min.js"></script>';
                                                echo "<script> Swal.fire({title:'Başarılı!', text:'Bilgileriniz Güncellendi', icon:'success', confirmButtonText:'Kapat'}).then((value) => {if (value.isConfirmed){window.location.href='useredit.php?id=".$_SESSION["id"]."'}})</script>";
                                            }
                                    }
                                    ?>
                                    <form class="form-horizontal" method="post" action="">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" value="<?= $sonuc["Isim"] ?>" id="Isim" name="txtIsim" placeholder="İsim Soyisim" required>
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="password" class="form-control" id="txtPass2" name="txtPass2" placeholder="Şifre Girin" required>
                                        </div>
